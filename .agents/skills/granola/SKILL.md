...
- **Granola desktop app** must be installed on macOS
- **Local cache** must exist at: `~/Library/Application Support/Granola/cache-v3.json`
- **Python 3** must be available in PATH (`python3`)
- **`Meetings/` folder** must exist in the vault root (`Meetings/`)

If Granola is not installed or the cache file is missing, inform the user clearly:
> "Granola cache not found at ~/Library/Application Support/Granola/cache-v3.json. Please install Granola desktop app and record at least one meeting before using this skill."
...
```bash
# List all meetings
python3 .Codex/skills/granola/scripts/granola.py list

# Get specific meeting with transcript
python3 .Codex/skills/granola/scripts/granola.py get <id>

# Sync new meetings to vault
python3 .Codex/skills/granola/scripts/granola.py sync

# Sync specific meeting
python3 .Codex/skills/granola/scripts/granola.py sync --id <id>
```
...
```bash
python3 .Codex/skills/granola/scripts/granola.py list
python3 .Codex/skills/granola/scripts/granola.py list --limit 5
```
...
```bash
python3 .Codex/skills/granola/scripts/granola.py get <id>
python3 .Codex/skills/granola/scripts/granola.py get <id> --no-transcript
```
...
python3 .Codex/skills/granola/scripts/granola.py sync --all
```
...
2. **Sync to vault**: `python3 .Codex/skills/granola/scripts/granola.py sync`
