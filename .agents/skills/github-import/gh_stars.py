...
def escape_yaml_string(s: str) -> str:
    ...
    Strips U+FE0E (text variation selector) — it forces text-style
    rendering of an emoji and breaks Quartz's OG image generation,
    which can't resolve `<codepoint>-fe0e` in its emoji map.
    """
    if not s:
        return ""
    return s.replace("︎", "").replace("\\", "\\\\").replace('"', '\\"')
