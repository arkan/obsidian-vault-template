...
description: >
  Generate a daily summary of all Claude Code activity by parsing session JSONL logs
  and writing an Obsidian-compatible markdown note into the vault. Use this skill whenever
  the user asks to summarize Claude Code activity, generate a daily Claude report, or when
  triggered by a scheduled cron. Also triggers for: "resume claude", "activite du jour",
  "what did Claude do today", "journal claude", "daily recap".
---

# Daily Claude Code Resume

Parse all Claude Code session logs from the last 24 hours and produce a single Obsidian note summarizing the work done across all projects.
...
Write to: `Areas/Claude/YYYY-MM-DD-claude.md`

Use today's date. If the directory `Areas/Claude/` does not exist, create it.
...
```markdown
---
title: "Claude Code Summary - YYYY-MM-DD"
date: YYYY-MM-DD
source: claude
type: daily/claude
tags:
...
Overview of Claude Code activity for the day (2-4 sentences). Mention the main projects, the overall work theme, and notable accomplishments. Use [[wikilinks]] for known entities.
