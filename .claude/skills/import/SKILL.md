...
Run `summarize` with `--cli claude` to generate a French summary:

```bash
summarize "<url_or_path>" --lang fr --cli claude --plain --length long 2>/dev/null
```
...
- Always run `summarize --extract` first, then `summarize --lang fr --cli claude` for the summary. Two separate calls.
...
After creating the note, append one line to `Areas/Claude/operations-log.md`:
```
